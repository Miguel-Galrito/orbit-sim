...
def lambert_universal(r1_km: np.ndarray, r2_km: np.ndarray, tof_s: float, mu: float = MU_EARTH_KM3_S2, prograde: bool = True) -> tuple[np.ndarray, np.ndarray]:
    ...
    if r1.shape != (3,) or r2.shape != (3,):
        raise ValueError("r1_km and r2_km must be length-3 vectors")
    if tof_s <= 0:
    ...
    sin_mag = np.linalg.norm(cross)/(r1m*r2m)
    ...
    if abs(A) < 1e-12:
        raise ValueError("Geometry is singular for 0/180 degree transfer")
    ...
    vals = [F(float(z)) for z in grid]
    bracket = None
    prev_z, prev_f = None, None
    for z, f in zip(grid, vals):
        if not np.isfinite(f):
            continue
        if prev_f is not None and f*prev_f <= 0:
            bracket = (prev_z, float(z)); break
        prev_z, prev_f = float(z), f
    if bracket is None:
        raise ValueError("No zero-revolution Lambert solution found for this geometry and time of flight")
    lo, hi = bracket
    for _ in range(200):
        z = 0.5*(lo+hi); fz = F(z)
        ...
        if abs(fz) < 1e-8:
            break
        flo = F(lo)
        if np.isfinite(flo) and flo*fz <= 0:
            hi = z
        else:
            lo = z
    yz = y(z)
    f = 1-yz/r1m; g = A*np.sqrt(yz/mu); gdot = 1-yz/r2m
    if abs(g) < 1e-12:
        raise ValueError("Lambert solution is numerically singular")
